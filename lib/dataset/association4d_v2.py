...
class Association4DV2(JointsDataset):
            ...
            for frame_idx in range(num_frames): # 遍历帧
                ...
                if frame_idx % self._interval== 0:
                    ...
                    for camera_name, proj in proj_dict.items():
                         # 处理当前相机（视野）
                        image_path=osp.join(seq,camera_name,f"{frame_idx+1}.jpg")
                        # 接下来加载此图片所对应的所有人的位姿
                        all_poses_3d = []
                        all_poses_vis_3d = []
                        all_poses = []
                        ...
                        for pose3d in pose3d_list:
                            ...
                            pose2d[:, :2] = project_pose3d_to_pose2d(self.cfg.TAG ,pose3d[:, :3], proj_dict[camera_name],width,height)
