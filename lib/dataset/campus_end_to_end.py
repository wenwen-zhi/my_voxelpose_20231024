...
class CampusEndToEnd(JointsDataset):
    ...
    def _get_db(self):
        ...
        # f=open("output/debug/campus.txt", 'a')
        if self.mode=="test":
            for i in self.frame_range: # for each frame
                for cam_name, cam in cameras.items(): # for each view(camera)
                    image = osp.join("Camera" + cam_name, "campus4-c{0}-{1:05d}.png".format(cam_name, i))
                    db.append({
