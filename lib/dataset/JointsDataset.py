...
def summary(name, data: np.ndarray):
    data = np.array(data)
    print(f"[{name}]\tshape: {data.shape}\tmin: {data.min()}\tmax: {data.max()}\tmean: {data.mean()}")
...
class JointsDataset(Dataset):
    ...
    def __init__(self, cfg, image_set, is_train, transform=None,mode=None):
        ...
        self.scale_factor = cfg.DATASET.SCALE_FACTOR # 这个参数没有用
        self.rotation_factor = cfg.DATASET.ROT_FACTOR # 这个参数没有用
    ...
    def generate_input_heatmap(self, joints):
        ...
        if self.target_type == 'gaussian':
            ...
            for n in range(nposes): # for each 位姿(人)
                ...
                cur_sigma = self.sigma * np.sqrt((human_scale / (96.0 * 96.0)))
                tmp_size = cur_sigma * 3
                for joint_id in range(num_joints): # for each 关键点
                    feat_stride = self.image_size / self.heatmap_size
                    mu_x = int(joints[n][joint_id][0] / feat_stride[0])
                    mu_y = int(joints[n][joint_id][1] / feat_stride[1])
                    ul = [int(mu_x - tmp_size), int(mu_y - tmp_size)]
